...
def sgd_optimizer(self, x, y, yaxis, plot):
    for i in range(self.max_itr):
        ...
        for j in range(0, self.m_sample, self.batch_size):
            ...
            if plot:
                z = int(i * np.ceil(self.m_sample / self.batch_size) + j / self.batch_size)
                yaxis[z] = self._cost_fun(x[j:j + self.batch_size, :].T, y[j:j + self.batch_size, :].T)
    ...
    for i in range(self.max_itr):
        ...
        for j in range(0, self.m_sample, self.batch_size):
            ...
            for k in range(len(self.paras)):
                ...
                temp_theta = self.paras[k] - self.learning_rate_init * self.deriva[k] / (np.sqrt(s[k]) + 1e-8)
                self.paras[k] = temp_theta
                self.affines[k].theta = temp_theta
            if plot:
                z = int(i * np.ceil(self.m_sample / self.batch_size) + j / self.batch_size)
    ...
    for i in range(self.max_itr):
        ...
        for j in range(0, self.m_sample, self.batch_size):
            ...
            for k in range(len(self.paras)):
                ...
                temp_theta = self.paras[k] - self.learning_rate_init * v_c / (np.sqrt(s_c) + 1e-8)
                self.paras[k] = temp_theta
                self.affines[k].theta = temp_theta
            if plot:
                yaxis[ep] = self._cost_fun(x[j:j + self.batch_size, :].T, y[j:j + self.batch_size, :].T)


OPTIMIZERS = {'sgd': sgd_optimizer, 'momentum': momentum_optimizer, 'rmsprop': rmsprop_optimizer,
              'adam': adam_optimizer}
