...
from base_functions import COSTFUNCTIONS
from base_functions import ACTIVATIONS
from utils import INITIALIZER
from base_functions import Linear
from preprocessing import ProcessingUnit
...
class Affine:
    ...
    def __init__(self, in_layer, out_layer, para_init, random_state=None):
        ...
        self.theta = self.initializer(self.in_layer.s, self.out_layer.s, random_state=random_state)
    ...
    def backward(self):
        ...
        self.in_layer.delta = np.multiply(np.dot(self.theta[:, 1:self.m].T, self.out_layer.delta),
                                          self.in_layer.gz)
...
class NeuralNetwork:
    ...
    def _initialize(self):
        ...
        for i in self.hidden_layers:
            ...
            affine = Affine(l_in, l_out, para_init=self.para_init, random_state=self.random_state)
            l_in = l_out
    ...
    def _fit_optimizer(self, x, y, yaxis, plot):
        ...
        for i in range(self.max_itr):
            ...
            for j in range(0, self.m_sample, self.batch_size):
                itr_num = int(i * np.ceil(self.m_sample / self.batch_size) + j / self.batch_size)
                self._cost_fun_derivative(x[j:j + self.batch_size, :].T, y[j:j + self.batch_size, :].T)
                for k in range(len(self.paras)):
                    updated_theta = None
                    if self.optimizer == 'adam':
                        v[k] = self.beta_1 * v[k] + (1 - self.beta_1) * self.deriva[k]
                        s[k] = self.beta_2 * s[k] + (1 - self.beta_2) * np.square(self.deriva[k])
                        corrected_v = v[k] / (1 - np.power(self.beta_1, itr_num + 1))
                        corrected_s = s[k] / (1 - np.power(self.beta_2, itr_num + 1))
                        updated_theta = self.paras[k] - alpha * corrected_v / (np.sqrt(corrected_s) + 1e-8)
                    elif self.optimizer == 'rmsprop':
                        s[k] = self.momentum * s[k] + (1 - self.momentum) * np.square(self.deriva[k])
                        updated_theta = self.paras[k] - alpha * self.deriva[k] / (np.sqrt(s[k]) + 1e-8)
                ...
                # loss curve option
                if plot:
                    yaxis[itr_num] = self._cost_fun(x[j:j + self.batch_size, :].T, y[j:j + self.batch_size, :].T)
            ...
            if self.learning_rate == 'expdecay':
                if (i + 1) % self.decay_step == 0:
                    alpha = np.power(self.decay_rate, (i + 1) / self.decay_step) * self.learning_rate_init
...
if __name__ == '__main__':
    ...
    nn = NeuralNetwork(hidden_layers=(172,), batch_size='auto', optimizer='adam', learning_rate='expdecay', lambd=0,
                       learning_rate_init=0.00585, decay_rate=0.98, decay_step=2, max_itr=500, shuffle=True,
                       verbose=True,
                       random_state=None)
